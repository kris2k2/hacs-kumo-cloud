...
class _BaseConfigFlow(config_entries.ConfigFlow):
    """Shared config flow implementation for Kumo Cloud."""
...
try:
    class ConfigFlow(_BaseConfigFlow, domain=DOMAIN):
        """Handle the config flow for Kumo Cloud on modern Home Assistant cores."""

        pass
except TypeError:  # pragma: no cover - fallback for older Home Assistant cores

    class ConfigFlow(_BaseConfigFlow):
        """Handle the config flow for Kumo Cloud on legacy Home Assistant cores."""
